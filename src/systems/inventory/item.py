...
class Item:
    ...
    def __post_init__(self):
        """Inicialização pós-criação do item."""
        # Validações básicas
        if self.current_stack > self.max_stack:
            self.current_stack = self.max_stack
        if self.durability and self.max_durability:
